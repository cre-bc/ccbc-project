...
router.post("/find", (req, res) => {
  ...
  const params = [];
  const sql =
    "select m_shohin_pk, shohin_code, shohin_nm1, shohin_nm2, coin  from m_shohin where delete_flg = '0' and shohin_code between '" +
    req.body.targetCode +
    "001' and '" +
    (req.body.targetCode + 1) +
    "999'" +
    " order by shohin_code desc";
  query(sql, params, res, req);
});
...
function mshohinInsert(tx, resdatas, req) {
  return new Promise((resolve, reject) => {
    var sql =
      "insert into m_shohin (shohin_code, shohin_nm1, shohin_nm2, coin, delete_flg, insert_user_id, insert_tm, update_user_id, update_tm) " +
...
function mshohinUpdate(tx, resdatas, req) {
  return new Promise((resolve, reject) => {
    var sql =
      "update m_shohin set shohin_code = ?, shohin_nm1 = ?, shohin_nm2 = ?, coin = ?," +
