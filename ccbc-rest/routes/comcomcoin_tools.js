...
async function findList(req, res) {
  ...
  for (var i = 0; i < resdatas.length; i++) {
    ...
    var resTrans = await selectBcTransactionList(db, resdatas[i].t_shain_pk, req.body.from_dt, req.body.to_dt)
    var trans = []
...
function selectBcTransactionList(db, shain_pk, from_dt, to_dt) {
  return new Promise((resolve, reject) => {
    ...
    var sql =
      "select *" +
      " from t_zoyo" +
      " where (zoyo_moto_shain_pk = " + shain_pk + " or zoyo_saki_shain_pk = " + shain_pk + ")" +
      " and nenji_flg in ('2', '3', '4')" +
      " and delete_flg = '0'" +
      " and insert_tm between '" + from_dt + "' and '" + to_dt + "'" +
      " order by insert_tm"
    db.query(sql, {
      type: db.QueryTypes.RAW
    })
